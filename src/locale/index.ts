...
const locale = (value) => ({
  ...
  ImageAnnotation: {
    ja: value?.alt
      ? `ここに<span class="${CLASS_NAME.highlightText}">「${value?.alt}」</span>の画像があります。`
      : 'ここに画像があります。',
    en: value?.alt
      ? `This image is about <span class="${CLASS_NAME.highlightText}">${value?.alt}</span>.`
      : 'There is a image',
    vi: value?.alt
      ? `Đây là hình ảnh về <span class="${CLASS_NAME.highlightText}">${value?.alt}</span>.`
      : 'Đây là một bức hình',
  },
  TableIntroduction: {
  ...
  TableOtherLine: {
    ja: `${value?.line}行目、${value?.rowData}${
      value?.isEndRow ? '、です。' : ''
    }`,
